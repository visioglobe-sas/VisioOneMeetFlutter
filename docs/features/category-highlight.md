...
getCategories: function (requestId) {
  ...
  sendToNative('categoriesLoaded', {
    requestId: requestId,
    categories: venue.categories.map(function (c) { return c.id; }),
  });
},
...
- **`venue.categories` entries are plain `{ id: string }` objects with no separate display label.** On the shared demo map used by this feature (`kDefaultMapHash`, no dedicated map needed here), those `id` values are opaque numeric strings (`"1"`, `"2"`, ... `"11"`, confirmed live — 10 categories, `"8"` absent from this particular map), not human-readable names — this demo has no choice but to display `category.id` as-is (see the chip labels in `CategoryHighlightOverlay`). A map authored with meaningful category IDs (e.g. `"restaurants"`) would display better out of the box, but the SDK itself provides no separate label field either way; a production app wanting real category names would need its own ID → label mapping, sourced outside the SDK.
